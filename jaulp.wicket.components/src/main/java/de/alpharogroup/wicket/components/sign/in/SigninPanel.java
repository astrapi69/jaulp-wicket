...
public class SigninPanel extends Panel {
	 ...
	 * provide their own version of a EmailTextField for the email.
	 *
	 * @param id the id
	...
	protected Component newEmailTextField(String id,  final IModel<SignInModel> model) {
		final IModel<String> labelModel = ResourceModelFactory.newResourceModel("global.email.label", this);
		final IModel<String> placeholderModel = ResourceModelFactory.newResourceModel("global.enter.your.email.label", this);
		final LabeledEmailTextFieldPanel<SignInModel> emailTextField = new LabeledEmailTextFieldPanel<SignInModel>(id, model, labelModel) {

		private static final long serialVersionUID = 1L;

		@Override
		protected EmailTextField newEmailTextField(final String id,
				final IModel<SignInModel> m) {
			final EmailTextField emailTextField = new EmailTextField(id, new PropertyModel<String>(model.getObject(), "email"));
	...
	/**
	 * Factory method for creating the EmailTextField for the password. This method is invoked in the
	 * constructor from the derived classes and can be overridden so users can
	 * provide their own version of a EmailTextField for the password.
	 *
	 * @param id the id
	 * @param model the model
	 * @return the text field
	...
	protected Component newPasswordTextField(String id, final IModel<SignInModel> model) {
		IModel<String> labelModel = ResourceModelFactory.newResourceModel("global.password.label", this);
		final IModel<String> placeholderModel = ResourceModelFactory.newResourceModel("global.enter.your.password.label", this);
		LabeledPasswordTextFieldPanel<SignInModel> pwTextField = new LabeledPasswordTextFieldPanel<SignInModel>(id, model, labelModel){
			...
			protected PasswordTextField newPasswordTextField(String id,
				...
				PasswordTextField pwTextField = new PasswordTextField(id, model(from(model).getPassword()));
