...
import org.jaulp.wicket.components.termofuse.cancellation.CancellationPanel;
import org.jaulp.wicket.components.termofuse.contract.ContractPanel;
import org.jaulp.wicket.components.termofuse.copyright.CopyrightPanel;
import org.jaulp.wicket.components.termofuse.dataprotection.DataProtectionPanel;
import org.jaulp.wicket.components.termofuse.disclaimer.DisclaimerPanel;
import org.jaulp.wicket.components.termofuse.fulfilmentandjurisdiction.FulfilmentAndJurisdictionPlacePanel;
import org.jaulp.wicket.components.termofuse.general.GeneralTermsAndConditionsPanel;
import org.jaulp.wicket.components.termofuse.legalreferences.LegalReferencesPanel;
import org.jaulp.wicket.components.termofuse.liability.LiabilityPanel;
import org.jaulp.wicket.components.termofuse.modificationsclause.ModificationsClausePanel;
import org.jaulp.wicket.components.termofuse.rightsandduties.RightsAndDutiesModel;
import org.jaulp.wicket.components.termofuse.rightsandduties.RightsAndDutiesPanel;
import org.jaulp.wicket.components.termofuse.salvatoriusclause.SalvatoriusClausePanel;
...
public abstract class TermOfUsePanel extends Panel {
	...
	public TermOfUsePanel(final String id, IModel<TermOfUseModel> model) {
		...
		add(generalTermsAndConditionsPanel = newGeneralTermsAndConditionsPanel("generalTermsAndConditionsPanel", Model.of(model.getObject().getGeneralTermsAndConditionsModel())));
		
		add(legalReferencesPanel = newLegalReferencesPanel("legalReferencesPanel", Model.of(model.getObject().getLegalReferencesModel())));

		add(contractPanel = newContractPanel("contractPanel", Model.of(model.getObject().getContractModel())));
		
		add(dataProtectionPanel = newDataProtectionPanel("dataProtectionPanel", Model.of(model.getObject().getDataProtectionModel())));
		
		add(copyrightPanel = newCopyrightPanel("copyrightPanel", Model.of(model.getObject().getCopyrightModel())));
		
		add(cancellationPanel = newCancellationPanel("cancellationPanel", Model.of(model.getObject().getCancellationModel())));
		
		add(liabilityPanel = newLiabilityPanel("liabilityPanel", Model.of(model.getObject().getLiabilityModel())));
		
		add(rightsAndDutiesPanel = newRightsAndDutiesPanel("rightsAndDutiesPanel", Model.of(model.getObject().getRightsAndDutiesModel())));
		
		add(modificationsClausePanel = newModificationsClausePanel("modificationsClausePanel", Model.of(model.getObject().getModificationsClauseModel())));

		add(salvatoriusClausePanel = newSalvatoriusClausePanel("salvatoriusClausePanel", Model.of(model.getObject().getSalvatoriusClauseModel())));

		add(fulfilmentAndJurisdictionPlacePanel = newFulfilmentAndJurisdictionPlacePanel("fulfilmentAndJurisdictionPlacePanel", Model.of(model.getObject().getFulfilmentAndJurisdictionPlaceModel())));

		add(disclaimerPanel = newDisclaimerPanel("disclaimerPanel", Model.of(model.getObject().getDisclaimerModel())));

	}
	
	protected Component newGeneralTermsAndConditionsPanel(String id, IModel<HeaderContentListModel> model) {
	...
	protected Component newFulfilmentAndJurisdictionPlacePanel(String id, IModel<HeaderContentListModel> model) {
