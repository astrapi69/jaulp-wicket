...
public class TextTemplateUtils
{
   ...
   * @param variables the map with the javascript options.
   * @param methodName The method name.
   * @return The generated javascript from the given map and the given method name.
   */
  public static String generateJavascriptTemplateContent(final Map<String, Object> variables, String methodName) {
...
public static void initializeVariables(final Map<String, Object> variables, final Set<StringTextValue<?>> allSettings) {
      ...
      if (!textValue.isDefaultValue()) {
        switch (textValue.getType()) {
          case STRING:
            ...
            TextTemplateUtils.setVariableWithSingleQuotationMarks(textValue.getName(), textValue.getValue(), variables);
            break;
          case ENUM:
            TextTemplateUtils.setVariableWithSingleQuotationMarks(textValue.getName(), ((ValueEnum)textValue.getValue()).getValue(), variables);
            break;
          case STRING_INTEGER:
            TextTemplateUtils.setVariableWithSingleQuotationMarks(textValue.getName(), textValue.getValue(), variables);
            break;
